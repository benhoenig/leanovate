...
- **What:** 3D rendering library — drives the live editor canvas, the Export View capture (4K PNG from current camera angle), and construction drawings
...
### Three.js (3D model rendering + canvas export)
...
- **Use cases:**
  ...
  3. **Export View (canvas → 4K PNG):** The live canvas's own `WebGLRenderer` is temporarily resized to 4K, renders one frame, `toBlob()`s to PNG, auto-downloads, then restores. No separate scene build, no `.glb` reload — the export is exactly what the designer sees. Implementation: `src/lib/exportCanvasView.ts`.

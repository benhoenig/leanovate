...
- **Note:** Three.js is used server-side for three separate purposes (see AI / Rendering Pipeline), but not in the live browser app.
...
- **Runs on:** Server-side batch script (Supabase Edge Function or CI), not in the live app
- **Three use cases:**
  1. **TRELLIS output rendering:** Takes the .glb file generated by TRELLIS and renders 4 isometric angle sprites from it. This is the second step of the AI pipeline — TRELLIS creates the 3D model, Three.js renders the 2D views.
...
- **Workflow (sprites):** .glb file (from TRELLIS or IKEA) → Three.js renders 4 isometric angles → upload sprites to Supabase Storage → link to catalog entries
...
- **Cost:** Free (open source, CPU-only rendering)
...
| **Three.js (in live app)** | Full 3D rendering in the browser is overkill for isometric views. PixiJS handles 2D sprites more efficiently. Three.js is used server-side only for rendering .glb files into sprites and generating room perspective previews (eye-level interior vignettes). |

...
- **Runs on:** Client-side (browser) for both sprite rendering and room perspective previews
- **Three use cases:**
  ...
  3. **Room perspective preview (eye-level interior vignette):** Loads all placed furniture .glb files into a 3D room shell (built from room geometry + finishes, with door/window cutouts), renders eye-level perspective images from user-selected wall positions. Wall selector lets user view from any wall. Used for client presentations to show mood and atmosphere. Implementation: `src/lib/renderRoomPreview.ts`.
...
- **Workflow (room preview):** Room geometry + finishes + all placed .glb files → browser Three.js renders eye-level interior vignette from user-selected wall → display in modal with wall selector + download/save to Supabase Storage (`rooms.preview_image_url`)

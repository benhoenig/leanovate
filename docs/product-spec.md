...
| **Picture frames with custom art** | Picture frame items track a real purchasable product (frame + price + mat opening dimensions). On placement the frame renders empty (just the frame's product photo with the blank mat visible). The designer clicks the frame in the right panel → "Choose art" → picks from their **Art Library** (images they've uploaded). Art starts private; the designer can flip the scope to "team" so everyone sees it (open share, no admin approval). The picker filters to aspect ratios that fit the mat opening within ±10% so landscape art doesn't get squished into portrait frames. The frame's purchase price lives on the frame item/variant; art is free/custom and not priced. |
| **Furniture rotation** | Click to rotate individual furniture items through 4 isometric directions on the canvas. |
| **Room perspective preview** | "Preview Room" button renders eye-level interior vignettes — realistic 3D perspective images of the current room showing all placed furniture (.glb models), room geometry (walls with door/window cutouts), and finishes. Wall selector buttons let the user view from any wall. Rendered client-side via Three.js (browser), displayed in a modal with download and save options. Used during client presentations to show mood and tone. |
...
15a. **Pick art for a placed picture frame** — As a designer, when I place a picture-frame item, I can click it in the right panel and pick an art image from my library to fill the frame's mat opening. I can upload new art on the spot, and the picker filters to aspect ratios that fit the frame. The cost summary still reflects the frame's purchase price; the art is free/custom.

15b. **Share art with the team** — As a designer, when I upload art I can flip its scope from "private" to "team" with a single toggle so every designer sees it in their picker. No admin approval — it's my call. I can flip it back to private at any time or delete it outright.
